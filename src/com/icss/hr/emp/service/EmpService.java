...
public class EmpService {
	...
	/**
	 * 
	 * 登录验证 返回1 用户名不存在 返回2 密码错误 返回3 登录成功
	 * 
	 * @throws SQLException
	 */
	...
	public int getEmpCountByCondition(Integer deptId, Integer jobId, String empName){
		...
		return mapper.getEmpCountByCondition(deptId, jobId, empName);
	}
	/**
	 * 条件搜索
	 */
	...
	public List<Emp> queryEmpByCondition(Integer deptId, Integer jobId, String empName,Pager pager){
		...
		return mapper.queryEmpByCondition(deptId, jobId, empName, pager);
	}
	/**
	 * 下载Excel报表
	 * @throws IOException 
	...
	public void writeExcel(List<Emp> list,OutputStream out) throws IOException{
		//工作簿
				...
				titleRow.createCell(3).setCellValue("员工邮箱");
				titleRow.createCell(4).setCellValue("员工电话");
				titleRow.createCell(5).setCellValue("员工部门");
				titleRow.createCell(6).setCellValue("员工职务");
				//数据行
				for(int i = 1;i <=list.size();i ++){
					...
					row.createCell(4).setCellValue(emp.getEmpPhone());
					row.createCell(5).setCellValue(emp.getDept().getDeptName());
					row.createCell(6).setCellValue(emp.getJob().getJobName());
